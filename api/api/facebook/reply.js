...
if (!admin.apps.length) {
  try {
    if (process.env.FIREBASE_CREDENTIALS_JSON) {
      ...
      console.log("Firebase Admin SDK initialized in reply function.");
    } else {
      throw new Error("FIREBASE_CREDENTIALS_JSON environment variable not set.");
    }
  } catch (error) {
    console.error("Error initializing Firebase Admin SDK in reply function:", error);
  }
}
...
async function getPageAccessToken(firebaseUserId) {
    ...
    if (docSnap.exists() && docSnap.data().fbPageAccessToken) {
        // لا نحتاج pageId للرد على تعليق، فقط accessToken
        return { accessToken: docSnap.data().fbPageAccessToken };
    }
...
module.exports = async (req, res) => {
  if (!admin.apps.length) {
    console.error("Firebase not initialized in reply handler.");
  ...
  const { firebaseUserId, commentId, message } = req.body;
  if (!firebaseUserId || !commentId || !message) {
    return res.status(400).json({ error: 'Firebase User ID, Comment ID, and message are required.' });
  }
  ...
  try {
    ...
    res.status(200).json({ success: true, replyId: response.data.id });
  } catch (error) {
    console.error('Error replying to Facebook comment:', error.response ? (error.response.data.error || error.response.data) : error.message);
    const errorMessage = error.response && error.response.data && error.response.data.error && error.response.data.error.message 
                       ? error.response.data.error.message 
                       : 'Failed to reply to Facebook comment.';
